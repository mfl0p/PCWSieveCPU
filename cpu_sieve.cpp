 ...
 */
int read_state_thread( searchData sd, uint64_t & current_p, int thread_id, bool & state_a, uint64_t & primecount, uint64_t & checksum)
{
	FILE *in;
...
void checkpoint_thread( searchData sd, uint64_t my_P, int thread_id, bool & state_a, uint64_t primecount, uint64_t checksum  )
{
	FILE *out;
	char buffer[20];	
...
void setupSearch(searchData & sd){
	...
	if (sd.nmin == 0)
	{
		...
		sd.nmin = 2*lg2(sd.pmin)-lg2(sd.kmax)-1;
	}
	...
	if((((uint64_t)1) << (64-sd.nstep)) > sd.pmin) {
		...
		if(pmin_1 < sd.pmin) sd.pmin = pmin_1;
...
//	-p 556439300e6 -P 556439440e6 -N 100000 -c
	...
	sd.cw = true;
	cpu_sieve( sd );
	if( sd.factorcount == 1 && sd.primecount == 4123452 && (sd.checksum == 0x9482721C21BCE80B) ){
...
//	-p42070000e6 -P42070050e6 -k 1201 -K 9999 -N 2000000			nstep 31
	...
	sd.cw = false;
	cpu_sieve( sd );
	if( sd.factorcount == 70 && sd.primecount == 1592285 && (sd.checksum == 0x7287CDBECFD30660) ){
