...
//	rcx += (ulong)(mad_hi(rax, (uint)N, (uint)((rax!=0)?1:0) ) );
	...
	m = _mm512_cmpeq_epi64_mask(rax, _mm512_setzero_si512());
	x = _mm512_add_epi64(rcx, _mm512_set1_epi64(1));
	rcx = _mm512_mask_blend_epi64(m, x, rcx);
...
//	rcx = (rcx>N)?(rcx-N):rcx;
	...
	x = _mm512_sub_epi64(rcx, N);
	rcx = _mm512_mask_blend_epi64(m, rcx, x);
...
//	rcx += (rax!=0)?1:0;
	__mmask8 mk = _mm512_cmpeq_epi64_mask(rax, _mm512_setzero_si512());
	__m512i add = _mm512_add_epi64(rcx, _mm512_set1_epi64(1));
	rcx = _mm512_mask_blend_epi64(mk, add, rcx);
...
//	rax = (rax>N)?rcx:rax;
	__mmask8 m = _mm512_cmpgt_epi64_mask(rax, N);
...
//	rcx = (rax!=0)?1:0;
	m = _mm512_cmpeq_epi64_mask(rax, _mm512_setzero_si512());
	rcx = _mm512_mask_blend_epi64(m, _mm512_set1_epi64(1), _mm512_setzero_si512());
...
void *thr_func_cw_avx512(void *arg) {
	...
	__m512i vP, vPs, vK, vK1, x, y, z, KPOS, theN, theK;
	__mmask8 m, me, mt;
	__m512i v_checksum		= _mm512_setzero_si512();
	const __m512i ZERO		= _mm512_setzero_si512();
	const __m512i ONE		= _mm512_set1_epi64(1);
	const __m512i ST		= _mm512_set1_epi64(63);
	const __m512i VNSTEP		= _mm512_set1_epi64(data->sd.nstep+1);
	...
	if(data->sd.test){
		...
		if( read_state_thread( data->sd, current_p, data->id, state_a, primecount, checksum ) ){
			printf("resuming from checkpoint in thread %d\n",data->id);
		}
	...
	while(!done){
		...
		if( ((int)ckpt_curr - (int)ckpt_last) > 60 ){
			...
			if(factorcount > 0){
				...
				resbuff[0] = '\0';
				for(uint32_t m=0; m < factorcount; ++m){
					uint64_t p = (factorP[m] < 0)?-factorP[m]:factorP[m];
					int s = (factorP[m] < 0)?-1:1;
					if ( sprintf( buffer, "%" PRIu64 " | %u*2^%u%+d\n",p,factorN[m],factorN[m],s) < 0 ){
						fprintf(stderr,"error in sprintf()\n");
						exit(EXIT_FAILURE);
					}
					strcat( resbuff, buffer );
				}
				report_solution_temp( resbuff, data->id );
				free(resbuff);
				factorcount = 0;
			}
		...
		do { 	
			...
			me	= _mm512_cmpeq_epi64_mask(x, ZERO);
			x	= _mm512_sub_epi64(vP, vK);
			KPOS	= _mm512_mask_blend_epi64(me, x, vK);				// kpos = (((uint)k0) & 1)?(my_P - k0):k0
			...
			mt	= _mm512_cmpgt_epi64_mask(VNSTEP,x);				// nstep+1 > ctz(kpos)?

			x	= _mm512_lzcnt_epi64(theK);
			y	= _mm512_lzcnt_epi64(theN);
			z	= _mm512_sub_epi64(x, y);

			m	= _mm512_cmpgt_epi64_mask(z, ZERO);				// clz(k) > clz(n), k < n?

			x	= _mm512_sub_epi64(theN, z);					// n -= shift
			theN	= _mm512_mask_blend_epi64(m, theN, x);		

			x	= _mm512_sllv_epi64(theK, z);					// k <<= shift
			theK	= _mm512_mask_blend_epi64(m, theK, x);		
...
//										printf("cw factor found, p %" PRIu64 " k %" PRIu64 " n %" PRIu64 " %d\n",P,the_k,the_n,s);
	...
	m = _mm512_cmpgt_epi64_mask(vP, _mm512_set1_epi64(data->pmax-1));
	x = _mm512_sub_epi64(v_checksum, vP);
	x = _mm512_sub_epi64(x, vK);
	v_checksum = _mm512_mask_blend_epi64(m, v_checksum, x);
...
void *thr_func_avx512(void *arg) {
	...
	while(!done){
		...
		do { 	
			...
			mt	= _mm512_cmpgt_epi64_mask(VNSTEP,x);				// nstep+1 > ctz(kpos)?
			m	= _mm512_cmpgt_epi64_mask(KMAX, theK);				// kmax+1 > k?
			mt	= _kand_mask8(m, mt);

			m	= _mm512_cmpgt_epi64_mask(theK, KMIN);				// K > kmin-1?
